...
class EigenfunctionsFromEigenvectors(Eigenfunctions):
    ...
    def weighted_outerproduct(
        ...
        **kwargs,
        ...
        '''
        This is the actual final crux that creates the kernel matrix 
        '''

        Phi_X = self.__call__(X, **kwargs)  # [N, J = num eigenvectors/functions] The (i,j) element contains the jth eigenfunctions output to the ith inputted vertex
        if X2 is None:
            Phi_X2 = Phi_X
        else:
    ...
    def phi_product(
        ...
        '''
        This doesn't include the weights. Not sure why this would be required. 
        '''
        if X2 is None:
            X2 = X
        Phi_X = self.__call__(X, **kwargs)  # [N, J]
