...
class MaternKarhunenLoeveKernel(BaseGeometricKernel):
    ...
    def __init__(
        ...
        #my eigens (RCQSTA):
        '''
        [ 0.          2.34314575  6.          6.          7.99999999  9.99999999
  9.99999999 13.65685424]
        '''
...
array([[0.        ],
    ...
    def K(
        ...
        weights = B.cast(B.dtype(params["nu"]), self.eigenvalues(params))  # [L, 1]. This casts the self.eigenvals(nums), which returns in vector form all of the phi-involved factors of the sum 
        Phi = self.eigenfunctions #This just fetches the eigenfunctions to use for later, (i.e to construct the second and third product in each summand)
        K = Phi.weighted_outerproduct(weights, X, X2, **kwargs)  # [N, N2] . This is where the sum really happens. 
        if is_complex(K):
            return B.real(K)
        else:
            return K
